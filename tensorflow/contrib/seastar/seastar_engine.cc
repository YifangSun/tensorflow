...
const static int kWaitTimeInUs = 100000;
...
void SeastarEngine::Server::start(uint16_t port,
  ...
  seastar::keep_doing([this, tag_factory] {
    return listener_->accept().then([this, tag_factory](seastar::accept_result ar) mutable {
...
SeastarEngine::SeastarEngine(uint16_t local,
                             ...
                             std::shared_ptr<SeastarPortMgr> seastar_port_mgr_p)
  ...
  seastar_port_mgr = seastar_port_mgr_p;
  ...
  app.run_deprecated(argc, argv, [&] {
          return server_.start()
                  .then([this] {
                      ...
                      return seastar_port_mgr->resolve_ips_update_cluster_spec().then([this]() {
