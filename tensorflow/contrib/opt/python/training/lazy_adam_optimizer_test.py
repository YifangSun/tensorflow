...
class AdamOptimizerTest(test.TestCase, parameterized.TestCase):
  ...
  def testMutableHashTableOfTensor(self):
    for dtype in [dtypes.float32, dtypes.float64]:
      with self.cached_session():
        ...
        var0 = lookup.MutableHashTable(dtypes.int64, dtype, [-1])
        var1 = lookup.MutableHashTable(dtypes.int64, dtype, [-1])

        indices0_np = np.array([0, 1], dtype=np.int64)
        indices1_np = np.array([0, 1], dtype=np.int64)
        var0.insert(constant_op.constant(indices0_np),
                    constant_op.constant(var0_np)).run()
        var1.insert(constant_op.constant(indices1_np),
                    constant_op.constant(var1_np)).run()
        grads0 = ops.IndexedSlices(
            constant_op.constant(grads0_np),
            constant_op.constant(indices0_np), constant_op.constant([2]))
        grads1 = ops.IndexedSlices(
            constant_op.constant(grads1_np),
            constant_op.constant(indices1_np), constant_op.constant([2]))
        opt = lazy_adam_optimizer.LazyAdamOptimizer()
        update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
        variables.global_variables_initializer().run()
        ...
        self.assertAllClose(
                [[1.0], [2.0]],
                var0.lookup(constant_op.constant(indices0_np)).eval())
        self.assertAllClose(
                [[3.0], [4.0]],
                var1.lookup(constant_op.constant(indices1_np)).eval())
  ...
  def testMutableHashTableOfTensorRepeatedIndices(self):
    for dtype in [dtypes.float32, dtypes.float64]:
      with self.cached_session():
        repeated_index_update_var = lookup.MutableHashTable(dtypes.int32,
                                                            dtype,
                                                            [-1])
        aggregated_update_var = lookup.MutableHashTable(dtypes.int32,
                                                        dtype,
                                                        [-1])
        ...
        variables.global_variables_initializer().run()
        self.assertAllClose(
                aggregated_update_var.lookup(
                        constant_op.constant(indices_np)).eval(),
                repeated_index_update_var.lookup(
                        constant_op.constant(indices_np)).eval())
