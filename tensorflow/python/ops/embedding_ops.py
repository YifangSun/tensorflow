...
def _embedding_lookup_and_transform(params,
  ...
  Args:
    ...
    transform_fn: An optional function to apply to each retrieved embedding.
      ...
      embeddings.
    id_insert_probability: probability to insert ids if not found.
    enable_fast_lookup: Enable fast and unsafe lookup when using
      MutableHashTable.
  ...
  with ops.name_scope(name, "embedding_lookup", params + [ids]) as name:
    ...
    if np == 1 and (not transform_fn or ids.get_shape().ndims == 1):
      with ops.colocate_with(params[0]):
        if is_hash_variable:
          if enable_fast_lookup:
            result = _clip(params[0].fast_lookup(ids),
                           ids, max_norm)
          else:
            result = _clip(
              params[0].lookup_and_insert(ids, id_insert_probability),
              ids, max_norm)
        else:
      ...
      for p in xrange(np):
        ...
        with ops.colocate_with(params[p]):
          if is_hash_variable:
            if enable_fast_lookup:
              result = params[p].fast_lookup(pids)
            else:
              result = params[p].lookup_and_insert(pids, id_insert_probability)
          else:
...
def embedding_lookup(
  ...
  return _embedding_lookup_and_transform(
      ...
      max_norm=max_norm,
      transform_fn=None,
      id_insert_probability=id_insert_probability,
      enable_fast_lookup=enable_fast_lookup)
