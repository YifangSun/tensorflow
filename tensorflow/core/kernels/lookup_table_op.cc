...
class MutableHashTableOfScalars final : public LookupInterface {
 public:
  ...
  Status ImportValuesAndAttrs(OpKernelContext* ctx, const Tensor& keys,
                              const Tensor& values,
                              const Tensor& update_ts,
                              const Tensor& attr_keys,
                              const Tensor& attr_flags,
                              const Tensor& attr_values) override {
    return DoInsert(true, keys, values);
  }
  ...
  Status ExportValuesAndAttrs(OpKernelContext* ctx) override {
    ...
    TF_RETURN_IF_ERROR(
        ctx->allocate_output("update_timestamp",
                             TensorShape({size}), &update_ts));
    TF_RETURN_IF_ERROR(
        ctx->allocate_output("attr_keys",
                             TensorShape(), &attr_keys));
    TF_RETURN_IF_ERROR(
        ctx->allocate_output("attr_flags", TensorShape(),
                             &attr_flags));
    TF_RETURN_IF_ERROR(
        ctx->allocate_output("attr_values", TensorShape(),
                             &attr_values));

    auto keys_data = keys->flat<K>();
    auto values_data = values->flat<V>();
    auto update_ts_data = update_ts->flat<uint64>();
    int64 i = 0;
    for (auto it = table_.begin(); it != table_.end(); ++it, ++i) {
      keys_data(i) = it->first;
      values_data(i) = it->second;
...
// If the given shape is a scalar return {1} instead. Otherwise leave it
// alone.
...
class MutableDenseHashTable final : public LookupInterface {
 public:
  ...
  Status CheckKeyAndValueTensorsForImport(const Tensor& keys,
    ...
    // The storage format in key_buckets_ and value_buckets_ is always
    // vectors,
    ...
    // Compute the final expected shape of the value by starting with the
    // shape
