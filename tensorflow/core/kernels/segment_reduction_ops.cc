...
struct UnsortedSegmentFunctor<CPUDevice, T, Index, InitialValueF, ReductionF> {
  void operator()(OpKernelContext* ctx, const Index num_segments,
    ...
    std::vector<std::vector<int64>> id_group(num_segments);
    for (int64 i = 0; i < N; ++i) {
      ...
      id_group[j].emplace_back(i);
    }

    int n = ctx->device()->tensorflow_cpu_worker_threads()->num_threads;
    thread::ThreadPool* thread_pool =
        ctx->device()->tensorflow_cpu_worker_threads()->workers;

    // 10000 is the MinCostPerShard from work_sharder.cc
    int estimate_shards =
        std::max<int>(1, std::min<int64>(static_cast<int64>(data_size / 10000),
                                         static_cast<int64>(n)));

    auto per_target_id_work = [&](int target_id) {
      std::vector<int64>& from_ids = id_group[target_id];
      if (from_ids.empty()) {
        output.template chip<0>(target_id).setConstant(InitialValueF()());
        return;
      }
      output.template chip<0>(target_id) =
          data_flat.template chip<0>(from_ids[0]);
      for (int i = 1; i < from_ids.size(); ++i) {
        reduction(data_flat.template chip<0>(from_ids[i]),
                  output.template chip<0>(target_id));
      }
    };

    BlockingCounter counter(estimate_shards);
    auto per_shard_work = [&](int shard_id) {
      for (int i = shard_id; i < num_segments; i += estimate_shards) {
        per_target_id_work(i);
      }
      counter.DecrementCount();
    };
    for (int i = 0; i < estimate_shards - 1; ++i) {
      thread_pool->Schedule([&, i] { per_shard_work(i); });
    }
    per_shard_work(estimate_shards - 1);
    counter.Wait();
  }
};
