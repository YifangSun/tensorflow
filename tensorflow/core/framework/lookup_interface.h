...
class LookupInterface : public ResourceBase {
 public:
  ...
  // Get the attribute with attr_key of keys in LookupTable.
  // Used for optimizer mainly

  // Returns the following statuses:
  // - OK: when the get finishes successfully.
  // - FailedPrecondition: if the table is not initialized.
  virtual Status GetAttr(OpKernelContext* ctx,
                         const Tensor& keys,
  ...
  // Set the attribute with attr_key of keys in LookupTable.
  // Used for optimizer mainly

  // Returns the following statuses:
  // - OK: when the get finishes successfully.
  // - FailedPrecondition: if the table is not initialized.
  virtual Status SetAttr(OpKernelContext* ctx,
                         const Tensor& keys,
                         const std::string& attr_key,
  ...
  // Keys not found in table will not be inserted. So no locks and faster in
  // serving.
  virtual Status FastFind(OpKernelContext* ctx, const Tensor& keys,
                          Tensor* values, const Tensor& default_value) {
    return Status::OK();
  }

  // If keys not found, insert with spec probability
  virtual Status FindAndInsert(OpKernelContext* ctx, const Tensor& keys,
                               Tensor* values, const Tensor& default_value,
  ...
  virtual Status ImportValuesAndAttrs(OpKernelContext* ctx, const Tensor& keys,
                                      const Tensor& values,
  ...
  virtual Status ImportFromFile(OpKernelContext *ctx,
                                const std::string &filename,
                                char delimiter) {
    return Status::OK();
  }

  virtual Status ExportValuesAndAttrs(OpKernelContext* ctx) {
    return Status::OK();
  }

  virtual Status EraseByAttrThreshold(const uint64& threshold) {
    return Status::OK();
  }
  ...
  virtual Status Remove(OpKernelContext* ctx, const Tensor& keys) {
    return Status::OK();
  }
